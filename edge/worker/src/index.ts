...
function bannedPage(
  ...
  const detail = isHack ? 'hacklemeye Ã§alÄ±ÅŸtÄ±nÄ±z' : reason;
  const ua = req.headers.get('user-agent') ?? 'unknown';
  const acceptLang = (req.headers.get('accept-language') ?? '').toLowerCase();
  const langCode = acceptLang.split(',')[0]?.split(';')[0]?.trim() ?? 'tr';
  const ref = req.headers.get('referer') ?? 'â€”';
  const cfRay = req.headers.get('cf-ray') ?? 'â€”';
  const cfAsn = (req as any).cf?.asn ?? req.headers.get('cf-asn') ?? 'â€”';
  const colo = (req as any).cf?.colo ?? 'â€”';
